...
else if(strcasecmp($_GET['method'], 'foursquare') == 0){
    $response['code'] = 1;
    $response['status'] = $api_response_code[$response['code']]['HTTP Response'];
    $devId = $_GET['device_id'];
    $sql = "Select did from Devices WHERE deviceID='$devId';";
    $strSQL = mysqli_query($connection,$sql);
    $Result = mysqli_fetch_array($strSQL);

    $sql = "Select * from Multify where did=".$Result['did'].";";
    $strSQL = mysqli_query($connection, $sql);
    $Result = mysqli_fetch_array($strSQL);
    $foursquare = $Result['4sqCheckInCount'];
    //$device = $Result['dCheckInCount'];
    //$difference = $foursquare - $device;

    $response['data'] = $foursquare;

}
else if(strcasecmp($_GET['method'], 'device') == 0){
    $response['code'] = 1;
    $response['status'] = $api_response_code[$response['code']]['HTTP Response'];
    $response['difference'] = 'OK';
}

else if(strcasecmp($_GET['method'], 'app') == 0){
    $response['code'] = 1;
    $response['status'] = $api_response_code[$response['code']]['HTTP Response'];
    $devId = $_GET['device_id'];
